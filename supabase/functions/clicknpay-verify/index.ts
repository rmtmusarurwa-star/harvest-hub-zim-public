...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      if (!cpRes.ok) {
        ...
        return json({
          ...
          error: `ClicknPay API error ${cpRes.status}`,
        });
      }
    ...
    const paymentStatus =
      cpStatus === "SUCCESS" ? "paid" : cpStatus === "FAILED" ? "failed" : "pending";
    ...
    const { data: pendingOrders, error: snapErr } = await supabase
      .from("orders")
      .select(
        "id, order_code, buyer_id, farmer_id, listing_title, quantity, unit, subtotal, total_amount",
      )
    ...
    if (paymentStatus === "paid" && pendingOrders && pendingOrders.length > 0) {
      ...
      if (obligations.length > 0) {
        ...
        if (oblErr) console.error("[clicknpay-verify] payout_obligations error:", oblErr.message);
          ...
          console.log(
            `[clicknpay-verify] created ${obligations.length} payout obligation(s) for ref ${primaryCode}`,
          );
      }
      ...
      const farmerMap = new Map<
        string,
        { listing_title: string; quantity: number; unit: string }[]
      >();
      ...
      for (const [farmerId, items] of farmerMap.entries()) {
        ...
        notifications.push({
          user_id: farmerId,
          type: "order",
          message: `Payment confirmed for ${summary}. Your payout is now incoming and waiting for Harvest Hub settlement.`,
          link: "/dashboard",
        });
      }
      ...
      if (adminId) {
        notifications.push({
          user_id: adminId,
          type: "announcement",
          message: `New pending seller payment: $${totalAmount.toFixed(2)} received via ClicknPay · Ref: ${primaryCode} · ${pendingOrders.length} item(s). Settle it in Pending Payments.`,
          link: "/admin",
        });
      }
    ...
    if (paymentStatus === "failed") {
      ...
      const related = (relatedOrders ?? []) as RelatedOrder[];
      const buyerIds = [...new Set(related.map((o) => o.buyer_id).filter(Boolean))];
      for (const buyerId of buyerIds) {
        failedNotifs.push({
          user_id: buyerId,
          type: "order",
      ...
      if (adminId) {
        const totalAttempted = related.reduce((s, o) => s + Number(o.total_amount ?? 0), 0);
        failedNotifs.push({
          user_id: adminId,
          type: "announcement",
          message: `Payment failed: $${totalAttempted.toFixed(2)} · Ref: ${primaryCode}. No seller payout is due.`,
          link: "/admin",
        });
      }
      ...
      if (failedNotifs.length > 0) {
        ...
        if (failErr)
          console.error("[clicknpay-verify] failed notification error:", failErr.message);
      }
    }
